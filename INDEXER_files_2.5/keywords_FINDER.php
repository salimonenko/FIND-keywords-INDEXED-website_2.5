...
$max_keywords_LEN = 0; $reg_keywords = ''; $message_to_user = ''; $ru_aff_FILE_NAME = '3'; $predlogi_final_PATH = ''; $ru_alfabet = ''; $enc_Arr = array();
...
// 2.1.4. Запускаем поиск по искомым (ключевым) словам
    find_keywords($keywords, $internal_enc, $reg_keywords, $message_to_user, $min_WORD_len, $metaphone_len, $path_DIR_name, $file_finded_FILES_name, $links_NUM_max_output, $INPUT_next_links, $ru_aff_FILE_NAME, $t0, $enc_Arr);
...
function find_keywords($keywords, $internal_enc, $reg_keywords, $message_to_user, $min_WORD_len, $metaphone_len, $path_DIR_name, $file_finded_FILES_name, $links_NUM_max_output, $INPUT_next_links, $ru_aff_FILE_NAME, $t0, $enc_Arr){
...
$indexes_Arr = array();
    for($j=0; $j < sizeof($keywords_Arr); $j++){ // По массиву, содерж. элементы логического выражения (например: слово1&&(слово2||слово3) )
        ...
        if($fuzzy === '1'){ // Для НЕчеткого поиска
            ...
            $keyword_forms_Arr = check_WORD_in_DIC($ru_aff_Arr, $internal_enc, $word, $metaphone_len, $path_DIR_name);
...
// 2.6. Объединяем множества индексов для каждого из искомых слов (сходных с изначальным словом) из $keyword_forms_Arr
    ...
    $bool_string = ''; // Будет что-то типа  $a[0] || $a[1] && $a[2] ...
