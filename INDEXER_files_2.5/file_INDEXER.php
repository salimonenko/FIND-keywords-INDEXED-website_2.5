...
// запускаем саму процедуру индексирования
    indexer($predlogi_PATH, $min_WORD_len, $internal_enc, $path_FILE_name_STRING, $ALL_files_Arr, $max_UNIX_saved, $DO_working_flag_FILE, $ENC_FILE_names_all_files, $enc_Arr, $total_size, $begins, $ends, $path_DIR_name, $metaphone_len, $path_FILE_max_UNIX, $str_UNIX_begin, $str_UNIX_end, $JS_manage_mes);
...
if(isset($_POST['sort_ru_dic']) && $_POST['sort_ru_dic'] === 'null'){
    ...
    DIC_sort($ru_dic_FILE_NAME_saved, $ru_dic_FILE_NAME, $path_DIR_name, $min_WORD_len, $metaphone_len, $DO_working_flag_FILE, $ru_dic_indexing_info_file);
...
echo '<input src="/LOCAL_only/INDEXER_files_2.5/imgs/indexing-files.png" style="background-image: none; vertical-align: middle; margin-left: 15px; width: 41px;" onclick="file_FINDER()" class="buttons_REDACTOR" title="Запустить индексирование ИМЕН всех файлов сайта" alt="Индексировать" type="image"><br/>';
    ...
    <div id="tmpID">
        <div style="display: inline-block; margin: 0; background-color: rgb(174, 255, 174); width: 100%">
            <div style="min-height: 40px; padding-left: 2px;">
                ...
                <div style="max-width: 45px; display: inline-block; float: right; ">

                    <input src="/LOCAL_only/INDEXER_files_2.5/imgs/sort1.png" style="background-image: none; vertical-align: middle; margin: 0 0 0 4px; width: 41px; height: 41px; float: right" onclick="ru_dic_SORTER('')" class="buttons_REDACTOR" title="Сортировка словаря. Будет заново создан и ПРОИНДЕКСИРОВАН словарь, отсортированный по алфавиту от А до Я. Старый файл сохранен под именем
<?php echo str_replace('\\', '/', $ru_dic_FILE_NAME_saved); ?>" alt="Сортировать" type="image"/>
                    <input id="DO_working" src="/LOCAL_only/INDEXER_files_2.5/imgs/go.png" onclick="DO_working(true, 0)" style="background-image: none; vertical-align: middle; margin: 0 0 0 4px; width: 41px; height: 41px; float: right" class="buttons_REDACTOR" title="Запуск индексирования СОДЕРЖИМОГО файлов, начиная с указанной или сохраненной на сервере строчки (из списка, содержащегося в файле
<?php echo realpath(PATH_FILE_NAMES_ALL_FILES); ?>)" alt=Индексирование" type="image"/>
                </div>

<div style="max-width: 45px; display: inline-block; float: right;">
                <input src="/LOCAL_only/INDEXER_files_2.5/imgs/indexing-files-ALL.png" style="background-image: none; vertical-align: middle; margin: 0 0 0 4px; width: 41px; height: 41px; float: right" onclick="file_FINDER('ALL')" class="buttons_REDACTOR" title="Запустить ЗАНОВО индексирование ИМЕН всех файлов сайта. Будет УДАЛЕН, а затем создан НОВЫЙ сводный файл-перечень со списком индексируемых файлов:
<?php echo realpath(PATH_FILE_NAMES_ALL_FILES); ?>" alt="Индексировать" type="image"/>
                <input src="/LOCAL_only/INDEXER_files_2.5/imgs/indexing-files.png" style="background-image: none; vertical-align: middle; margin: 0 0 0 4px; width: 41px; height: 41px; float: right" onclick="file_FINDER(null)" class="buttons_REDACTOR" title="Запустить индексирование ИМЕН только НОВЫХ или ИЗМЕНЕННЫХ файлов сайта. Будет ОБНОВЛЕН (и/или дополнен) уже имеющийся сводный файл-перечень со списком индексируемых файлов:
<?php echo realpath(PATH_FILE_NAMES_ALL_FILES); ?>" alt="Индексировать" type="image"/>

</div>
                <input src="/LOCAL_only/INDEXER_files_2.5/imgs/del-dir.png" style="background-image: none; vertical-align: middle; margin: 0 0 0 4px; width: 41px; height: 41px; float: right" onclick="del_endexed_FILES_DIRS('')" class="buttons_REDACTOR" title="Удалить ВСЕ индексные файлы (вместе с их каталогами)? Т.е. будет полностью, со всеми вложенными подкаталогами и файлами, УДАЛЕН каталог:
<?php echo str_replace('\\', '/', $path_DIR_name); ?>" alt="УДАЛИТЬ индексные файлы" type="image"/>
                Файл: <span id="file_name" style="font-weight: bold; ">Ни один из файлов не индексируется!... Возможно, все файлы были проиндексированы.</span></div>
        ...
        <div style="display: inline-block; padding-top: 17px; margin-bottom: 10px; vertical-align: top;">
            ...
            <div style="display: inline-block; position: relative">
                ...
                <!--<input id="save" src="/LOCAL_only/INDEXER_files_2.5/imgs/save_as2.png" style="background-image: none; width: 25px; vertical-align: middle; margin-left: 15px;" class="buttons_REDACTOR" title="Сохранить заданный номер строки (в файле files.txt): с этого номера будет начато/продолжено индексирование файлов сайта" alt="SAVE->" type="image"/>-->
            ...
            <div style="display: inline-block"><input id="DO_working_stop" src="/LOCAL_only/INDEXER_files_2.5/imgs/Close-Cancel.png" onclick="DO_working(false, 0)" style="background-image: none; vertical-align: middle; position: relative; top: -5px; max-width: 41px" class="buttons_REDACTOR" title="Остановить выполняемую операцию" alt="Стоп" type="image"/></div>

             <div style="display: inline-block"><input id="DO_working" src="/LOCAL_only/INDEXER_files_2.5/imgs/go_refresh.png" onclick="DO_working(true, 1)" style="background-image: none; vertical-align: middle; position: relative; top: -5px; max-width: 41px" class="buttons_REDACTOR" title="Запуск индексирования СОДЕРЖИМОГО обновленных/новых файлов (из списка, содержащегося в файле
<?php echo realpath(PATH_FILE_NAMES_ALL_FILES); ?>)" alt="Запуск" type="image"/></div>

            <div lang="ru" class="hyphens" style=" font-size: 90%; background-color: #AEFFF4; text-align: center">Показать номера НЕпроиндексированных строк:<input src="/LOCAL_only/INDEXER_files_2.5/imgs/find.png" style="background-image: none; width: 25px; vertical-align: middle; margin-left: 15px;" class="buttons_REDACTOR" title="Показать номера строк из файла files.txt, которые еще не были проиндексированы" onclick="find_NOT_INDEXED_strings()" alt="FIND->" type="image"/></div>
            </div>
        </div>
<p style="display: inline-block">Всего файлов, которые будут индексироваться: <span id="last_index"><?php echo sizeof($ALL_files_Arr); ?></span></p>
        <input id="Notepad" src="/LOCAL_only/INDEXER_files_2.5/imgs/notepad_pp-40.png" style="background-image: none; vertical-align: middle; margin-left: 15px;" class="buttons_REDACTOR" title="Открыть файл со списком индексируемых файлов Notepad++
<?php echo realpath(PATH_FILE_NAMES_ALL_FILES); ?>" alt="Открыть файл со списком индексируемых файлов Notepad++" type="image"/>

        <input src="/LOCAL_only/INDEXER_files_2.5/imgs/error_log.png" style="background-image: none; vertical-align: middle; margin-left: 15px; float: right; width: 41px" class="buttons_REDACTOR" title="Открыть лог-файл  журнал ошибок" onclick="open_error_log()" alt="Открыть файл ошибок" type="image"/>


<div style="overflow: auto;">
    <div style="float: right; background-color: #74E36D; padding: 15px 0px 0px 30px; margin-top: 20px">
        <input src="/LOCAL_only/INDEXER_files_2.5/imgs/find-files-indexed.png" style="background-image: none; float: right; z-index: 2; vertical-align: middle; width: 41px;" onclick="show_hide_POPUP(['popup0', 'popup1'], null)" class="buttons_REDACTOR" title="Найти файлы по искомым словам" type="image"/>
        <input id="fuzzy" src="/LOCAL_only/INDEXER_files_2.5/imgs/find-files-.png" style="background-image: none; float: right; z-index: 2; vertical-align: middle; height: 20px; width: 20px; margin-top: 21px;" onchange="fuzzy_finding('fuzzy', 'onchange')" class="buttons_REDACTOR" title="Поставьте галочку, чтобы использовать нечеткий поиск" type="checkbox"/>
    </div>
    <p style="margin-bottom: 0px;">Общий объем проиндексированных файлов за последнюю <br/>операцию: <span id="total">0</span> кБ</p>
        ...
        <div style="position: absolute; right: -4px; bottom: -30px;"><div><input src="/LOCAL_only/INDEXER_files_2.5/imgs/delete-40_res.png" style="background-image: none; width: 25px" class="buttons_REDACTOR" title="Очистить журнал сообщений (ниже)" onclick="clean_responser('responser')" type="image"/></div></div>
...
// Функция удаляет все индексные каталоги/файлы (и сам каталог metaphones)
...
//            const source = new EventSource(events_php);
        ...
        source.onerror = function (error) { // Ошибка может возникнуть, например, когда скрипт на сервере, обслуживающий EventSource, прекратит работу
            ...
            console.log(error);
            source_close('Соединение EventSource прекращено, т.к. была попытка установить повторное (автоматическое) соединение или, возможно, сервер прервал соединение EventSource.');
        };
...
function indexer($predlogi_PATH, $min_WORD_len, $internal_enc, $path_FILE_name_STRING, $ALL_files_Arr, $max_UNIX_saved, $DO_working_flag_FILE, $ENC_FILE_names_all_files, $enc_Arr, $total_size, $begins, $ends, $path_DIR_name, $metaphone_len, $path_FILE_max_UNIX, $str_UNIX_begin, $str_UNIX_end, $JS_manage_mes){
...
$time_UNIX_i_MAX = 0; // Для начала
    ...
    for($i=$i_begin; $i < sizeof($ALL_files_Arr); $i++){

// 7.1.   /* Проверяем, присутствует ли флаговый файл. Если да, то делаем следующую итерацию. Если нет - прекращаем цикл */
        if(!file_exists($DO_working_flag_FILE)){
...
// Перед прерыванием сохраняем макс. метку времени UNIX в файл-перечень files.txt
                ...
                $mess = 'Индексирование прекращено.';
            }else{ // Если страница была просто обновлена
                $mess = 'Для начала/продолжения индексирования файлов сайта нужно произвести ЗАПУСК индексирования. Индексирование начнется с номера строки (в файле files.txt), который задан в панели слева.';
            }
        ...
        if(connection_aborted()){
            sendMsg(time(), 'Похоже, прервано соединение с сервером. Процесс индексации словаря остановлен. Попробуйте повторить эту операцию.', true);
            die(); //
        }
...
// 7.2. Из files.txt получаем имя файла, который (возможно) будет индексироваться
        $file_name_Arr = explode(';', $ALL_files_Arr[$i]);
...
// 7.4. Сохраняем максимальную метку времени UNIX во временный файл
        ...
        // Перед начало индексирования очередного файла это значение записывается в файл-перечень files.txt
...
// 7.5. Имя файла перекодированное (только для вывода в виде информации на экран). Актуально, когда в имени файла содержится, например, кириллица
        ...
        if(!is_readable($file_name_ABS)){ // Если файл вдруг не существует (мало ли...) или заблокирован в данный момент
            ...
            file_put_contents(PATH_FILE_NAMES_ERROR, $file_name. '|'. array_search($ALL_files_Arr[$i], $ALL_files_Arr). '|'. $mess.' '.date("d.m.Y - H:m:s") . PHP_EOL , FILE_APPEND);
            continue;
        }else{
...
// 7.12. Создаем индекс-каталоги, в них создаем индекс-файлы, а в последние записываем индекс-номера путей к файлам сайта (если таких номеров еще не было записано)
        if(($number = array_search($ALL_files_Arr[$i], $ALL_files_Arr)) !== false){ // Определяем число-индекс, присутствующий у имени файла (из содержимого которого выше были получены слова-метафоны), после |  (в файле files.txt)
            ...
            for($k=0; $k < sizeof($body_Arr); $k++){ // По каждому из слов
                ...
                if($rez === 'continue'){ // По идее, это лишнее +++
                    continue;
                }
