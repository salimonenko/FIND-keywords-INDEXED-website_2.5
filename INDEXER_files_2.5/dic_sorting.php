...
function DIC_sort($ru_dic_FILE_NAME_saved, $ru_dic_FILE_NAME, $path_DIR_name, $min_WORD_len, $metaphone_len, $DO_working_flag_FILE, $ru_dic_indexing_info_file)
{
...
for($i=1; $i < sizeof($dic_Arr); $i++){ // Первый элемент этого массива - цифра - общее число слов словаря. Ее не индексируем
        ...
        if(!file_exists($DO_working_flag_FILE)){
            $mess = '<p class="info_mes"><b>Индексирование файла-словаря остановлено.</b></p>';
            die($mess);
...
$rez = LAST_met_2_index($keyword_metaph, $dic_word_SUFF, true, $path_DIR_name, '', array(), ''); // Та же самая функция, к-рая используется для индексации (содержимого, т.е. слов) файлов сайта
